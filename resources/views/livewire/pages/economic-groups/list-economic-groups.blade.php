<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
    <div class="p-2 sm:p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex justify-between items-center">
        <form method="GET" action="{{ route("economic-groups.search") }}" >
            <div class="max-w-2xl flex justify-start gap-2">
                <x-input-label for="q" value="Nome do grupo" class="sr-only" />
                <x-text-input
                    wire:model="q"
                    id="q"
                    name="q"
        ...
        <a href="{{ route('add-group') }}" wire:navigate class="flex justify-center items-center h-10 w-10 rounded-lg dark:bg-gray-900 dark:text-white hover:bg-gray-400 hover:text-gray-900 transition-all hover">
    ...
    <div class="p-2 sm:p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
        @livewire('data-table', [
            ...
            'rows' => $groupsFound,
            'link' => "group",
            'deleteLink' => "economic-groups.destroy"
        ])
    </div>
</div>