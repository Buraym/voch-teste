<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
    <div class="p-2 sm:p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex justify-between items-center">
        <form method="GET" action="{{ route("units.search") }}" >
            <div class="max-w-2xl flex justify-start gap-2">
                <x-input-label for="q" value="Nome da unidade" class="sr-only" />
                <x-text-input
                    wire:model="q"
                    id="q"
                    name="q"
        ...
        <a href="{{ route('add-unit') }}" wire:navigate class="flex justify-center items-center h-10 w-10 rounded-lg dark:bg-gray-900 dark:text-white hover:bg-gray-400 hover:text-gray-900 transition-all hover">
    ...
    <div class="p-2 sm:p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
        @livewire('data-table', [
            'columns' => ['ID', 'Nome Fantasia', 'Razão Social', 'CNPJ', 'Bandeira'],
            'rows' => $unitsFound,
            'link' => "unit",
            'deleteLink' => "units.destroy"
        ])
    </div>
</div>